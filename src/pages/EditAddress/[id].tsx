...
type EditAddresProps = {
  data: AllAddressType;
};
export default function EditAddress({ data }: EditAddresProps) {
  const {
    query: { id },
  } = useRouter();
  ...
  return (
    <styled.EditAddress>
      <Navigation text="Editar Endereço" />
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  try {
    const response = await api.get(`/address/readOne/${ctx.params?.id}`, {
